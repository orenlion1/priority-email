...
- Gmail OAuth is configured through Google Cloud project `priority-email-500114`.
...
- Slack app `Priority Email` is installed in workspace `ensemble-grafana` with `chat:write`.
...
- AWS access uses the local AWS CLI profile `ensemble-grafana` for account `629513454417`; static AWS access keys must not be copied into this repo.
...
- Sender email domain, such as `@grafana.com`.
...
- A domain filter may be entered with or without a leading `@`; for example, `grafana.com` and `@grafana.com` should be treated as equivalent.
...
- Given a connected Gmail account and an active domain filter for `@grafana.com`, when a new email arrives from `person@grafana.com`, then the user receives a phone push notification and a Slack summary is posted with a link to the email.
...
- Given a connected Apple iCloud Mail account and an active domain filter for `example.com`, when a new email arrives from `person@example.com`, then the user receives a phone push notification and a Slack summary is posted with a link to the email when available.
