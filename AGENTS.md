...
- For functional runtime changes, push the source commit to GitHub and wait for CI to pass on `main`; the `Deploy` GitHub Actions workflow then builds the Lambda zip and runs `update-function-code` for the CI-passing commit. Operator bootstrap of the runtime secret uses `scripts/aws/bootstrap-aws.sh` locally; infra is `terraform -chdir=infra/terraform apply` (a fresh checkout must first `terraform -chdir=infra/terraform init -backend-config="bucket=ensemble-grafana-tf-state-<account>" -backend-config="region=us-east-1" -backend-config="use_lockfile=true"` — the bucket/region are not committed). Verify the live function version and a poll cycle after deployment.
