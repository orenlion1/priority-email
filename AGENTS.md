...
- The poller is a scheduled AWS Lambda (Python 3.13, EventBridge every 5 min) since 2026-07-10; it replaced the EKS pod after the ensemble-grafana cluster was decommissioned. Provisioning is `infra/terraform`; the poller's runtime state and filters live in the app's own `priority-email-state-<account>` S3 bucket, while Terraform backend state lives in the shared `ensemble-grafana-tf-state-<account>` bucket (key `stacks/priority-email/terraform.tfstate`, S3-native `use_lockfile` locking) that core-infra owns — the same bucket winnow uses.
- For functional runtime changes, push the source commit to GitHub and wait for CI to pass on `main`; the `Deploy` GitHub Actions workflow then builds the Lambda zip and runs `update-function-code` for the CI-passing commit. Verify the live function version and a poll cycle after deployment.
- For infrastructure changes (`infra/terraform/**`), do NOT `terraform apply` from a laptop — the `Infra Apply` workflow runs Terraform in CI under scoped OIDC roles (orenlion1 standard, core-infra `docs/standards/terraform-ci-oidc.md`): read-only plan posted on the PR, write apply on `main` gated by the required-reviewer `terraform-apply` environment. Lambda code stays on the `Deploy`/`update-function-code` path (Terraform ignores the function's zip). The operator's only Terraform task is the one-time bootstrap that creates the CI roles (see CLAUDE.md); runtime-secret bootstrap stays `scripts/aws/bootstrap-aws.sh` locally.
- For every notification configuration update, run the unit tests; after they pass and the commit reaches `main`, the `Deploy` workflow rolls out the CI-passing commit automatically. Verify the rollout and changed live configuration without printing private filter values or secrets.
- For filter-only updates, never commit plaintext values. Append an age-encrypted operation with `scripts/filters/add-filter-op.sh <add|remove> <domain|email-address|sender-name> <value>`, commit the new `filters/ops/*.age` file, and push to `main`. Once CI passes, the `Deploy` workflow's `sync-filters` job assembles the filters and uploads them to the S3 filter store (checksum-verified) — no local machine or human approval is involved. Never leave a successful notification update waiting for a future deployment.
