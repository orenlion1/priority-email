...
- The poller is a scheduled AWS Lambda (Python 3.13, EventBridge every 5 min) since 2026-07-10; it replaced the EKS pod after the ensemble-grafana cluster was decommissioned. Provisioning is `infra/terraform`; state and filters live in the `priority-email-state-<account>` S3 bucket.
